...
| `multer: ^2.3.0` | Clears [GHSA-wc9g-mqfw-jrwm](https://github.com/advisories/GHSA-wc9g-mqfw-jrwm), [GHSA-qfvm-cv95-jqjf](https://github.com/advisories/GHSA-qfvm-cv95-jqjf) and [GHSA-535w-7cp7-47q4](https://github.com/advisories/GHSA-535w-7cp7-47q4) (high, DoS) plus [GHSA-qvfw-j98x-7q72](https://github.com/advisories/GHSA-qvfw-j98x-7q72) (high, file size limit bypass via async `fileFilter` race), affecting ≤ 2.2.0. `@nestjs/platform-express@12` pins `multer 2.2.0` exactly. Remove when `@nestjs/platform-express` resolves ≥ 2.3.0 on its own. |
| `mailparser: ^3.9.26` | Clears [GHSA-8m3c-c648-2xjj](https://github.com/advisories/GHSA-8m3c-c648-2xjj), [GHSA-wmmp-3585-3rmp](https://github.com/advisories/GHSA-wmmp-3585-3rmp), [GHSA-2x7j-588g-ccc2](https://github.com/advisories/GHSA-2x7j-588g-ccc2) and [GHSA-cc9r-2j5m-2m83](https://github.com/advisories/GHSA-cc9r-2j5m-2m83) (all high, affecting nodemailer ≤ 9.1.0). `preview-email@3.4.0` — reached via `@nestjs-modules/mailer` — pins `mailparser 3.9.17` exactly, which ships `nodemailer 9.0.6`; `mailparser 3.9.26` is the release that pairs with `nodemailer 10.0.9` upstream. `preview-email`'s own `nodemailer@9.1.1` is already patched, so no nodemailer override is needed. Remove when `preview-email` resolves a mailparser ≥ 3.9.26 on its own. |
...
**Removed pins:** `less: ^4.9.0` was dropped. `@nx/webpack` caps `less <4.9.0`, but less 4.8.x — what that range resolves to — no longer pulls the vulnerable `image-size` chain, so `npm audit` is clean without a pin (the repo also contains no `.less` files, so the compiler version is inert). Do not re-add a less pin unless a future less release starts pulling a vulnerable transitive again.
