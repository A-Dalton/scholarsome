...
| `nodemailer: ^10.0.9` | Clears [GHSA-8m3c-c648-2xjj](https://github.com/advisories/GHSA-8m3c-c648-2xjj), [GHSA-wmmp-3585-3rmp](https://github.com/advisories/GHSA-wmmp-3585-3rmp), [GHSA-2x7j-588g-ccc2](https://github.com/advisories/GHSA-2x7j-588g-ccc2) and [GHSA-cc9r-2j5m-2m83](https://github.com/advisories/GHSA-cc9r-2j5m-2m83) (all high, affecting ≤ 9.1.0). `mailparser@3.9.17` — reached via `@nestjs-modules/mailer` → `preview-email` — pins `nodemailer 9.0.6` exactly. Remove when that chain resolves a patched nodemailer on its own. |
| `mailparser: ^3.9.26` | Companion to the nodemailer override above: `preview-email@3.4.0` pins `mailparser 3.9.17` exactly, which ships with `nodemailer 9.0.6`. `mailparser 3.9.26` is the release that pairs with `nodemailer 10.0.9` upstream, so forcing both keeps the pair consistent. Remove together with the nodemailer override once upstream resolves fixed versions on its own. |
